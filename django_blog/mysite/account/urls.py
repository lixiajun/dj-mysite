#coding=utf-8
from django.conf.urls import url
from . import views

from django.conf import settings

from django.contrib.auth import views as auth_views  #django内置的认证视图文件

urlpatterns = [
    #url('^login/$', views.user_login, name="user_login"),
    url(r'^login/$', auth_views.login, name='user_login'), #django内置的登录,登录成功后会自动跳转，需要设置LOGIN_REDIRECT_URL
    url(r'^logout/$', auth_views.logout, {"template_name": "account/logout.html"}, name="user_logout"),  #使用内置的退出模块，自己定义退出的页面
    url(r'^register/$', views.register, name="user_register"),

    #更改密码模块
    url(r'^password-change/$', auth_views.password_change, {"post_change_redirect": "account/password-change-done/"},
        name='password_change'),  #向函数中传递参数
    url(r'^password-change-done/$', auth_views.password_change_done, name='password_change_done'),
    url(r'^password-reset/$', auth_views.password_reset,{"template_name": "account/password_reset_form.html",
                                                         "email_template_name": "account/password_reset_email.html",
                                                         "subject_template_name": "account/password_reset_subject.txt",
                                                         "post_reset_redirect": "/account/password-reset-done",}, #绝对路径，
                                                        name="password_reset"),
    url(r'password-reset-done/$', auth_views.password_reset_done, {"template_name": "account/password_reset_done.html"},
        name="password_reset_done"),
    url(r'password-reset-confirm/(?P<uidb64>[-\w]+)/(?P<token>[-\w]+)/$', auth_views.password_reset_confirm,
        {"template_name": "account/password_reset_confirm.html","post_reset_redirect":"/account/password-reset-complete"}, #绝对路径
        name="password_reset_confirm"), #绝对路径和相对路径的区别是，相对路径是在原路由上继续加上去，绝对路径是继续加上原路由
    url(r'^password-reset-complete/$', auth_views.password_reset_complete,
        {"template_name": "account/password_reset_complete.html"},
        name="password_reset_complete"),

    #个人信息
    url(r'^my-information/$', views.myself, name="my_information"),
    url(r'^edit-my-information/$', views.myself_edit, name="edit_my_information"),
    url(r'^my-image/$', views.my_image, name="my_image"),
]